...
"""
import os, re, uuid, json, time, logging
from fastapi import FastAPI, Request, Response, Query
from fastapi.responses import PlainTextResponse, JSONResponse
import gradio as gr

from app.security import safe_log, verify_hmac_signature
...
def handle_message_logic(wa_id: str, text: str, has_media: bool = False, msg_type: str = "text", wamid: str = None):
    ...
    try:
        ...
        if suppressed:
            ...
            if cmd in ("start", "subscribe", "resubscribe", "unstop"):
                conn.execute("DELETE FROM suppressions WHERE wa_id=?", (wa_id,))
                conn.commit()
            else:
                if cmd not in ("stop", "unsubscribe"):
                    conn.close()
                    # Customer wrote to us first, so a one-line reminder is allowed. Log it (wamid PK = no duplicate replies on Meta retries).
                    content = safe_log(text) if text else ("[MEDIA]" if has_media else "")
                    ok, insert_msg = insert_message(wamid, wa_id, content, "suppressed", "reminder_unsubscribed", "LOW", status="received")
                    return {
                        "wamid": wamid,
                        "intent": "suppressed",
                        "action": "reminder_unsubscribed",
                        "priority": "LOW",
                        "reply": "You are currently unsubscribed from Meds For Less messages. Reply START to subscribe again.",
    ...
    # Bot replies 24/7 — no office-hours gating
    ...
    logger.info(f"Message {safe_log(wamid)} from {safe_log(wa_id)} intent={intent} priority={priority} insert={insert_msg}")
...
def health():
    stats = get_stats()
    return {"status": "ok", "phase": "1 v1.1", "intents": 22, "stats": stats, "availability": "24/7", "db": get_db_path()}
        ...
        for entry in payload.get("entry", []):
            for change in entry.get("changes", []):
                ...
                for msg in messages:
                    ...
                    if result.get("reply") and result.get("ok"):
...
with gr.Blocks(theme=gr.themes.Soft(primary_hue="blue"), title="Medsforless Phase 1 v1.1 — 22 QA Intents") as demo:
    ...
    gr.Markdown("**Scope:** Webhook auth (HMAC), queue (<200ms), DB (wamid PK), FAQ auto (5 intents), Human Escalation (17 intents), 24/7 Bot Replies, Suppression Table, Audit Logs | **Exit Criteria:** No duplicate replies, Verified redacted logs, Replay tests")
